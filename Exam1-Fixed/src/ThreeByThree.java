...
public class ThreeByThree {
	...
	public ThreeByThree(int i, int j, Color gray) {
		this.ptx = i;
		this.pty = j;
		color = gray;
	...
	public void drawOn(Graphics2D g) {
		...
		for (int i= 0;i<3;i++){
			for (int j = 0;j<3;j++){
				...
				g.fillRect(0+30*i, 0+30*j, SQUARE_WIDTH, SQUARE_WIDTH);
				g.setColor(Color.BLACK);
				g.drawRect(0+30*i, 0+30*j, SQUARE_WIDTH, SQUARE_WIDTH);
			}
		...
		for (int i = 0;i<3;i++){
			for (int j = 0;j<3;j++){
				...
				g.fillRect(this.ptx+30*i, this.pty+30*j, SQUARE_WIDTH, SQUARE_WIDTH);
				g.setColor(Color.BLACK);
				g.drawRect(this.ptx+30*i, this.pty+30*j, SQUARE_WIDTH, SQUARE_WIDTH);
			}
		}
		
	}

	public void setCellColor(int i, int j, Color color) {
...
public class SmallProblems {
	...
	public static ArrayList<Integer> interchange(int[] input1, int[] input2) {
		...
		for (int i = 0; i < input1.length; i++) {
			...
			output.add(2*i, input1[i]);
			output.add(2*i+1, input2[input2.length-i]);
	...
	public static ArrayList<String> getSubrangeStrings(String input) {
		...
		for (int i = 0; i < input.length(); i++) {
			for (int j = i+1; j < input.length()-i; j++) {
				result.add(i+j-1, input.substring(i, j));
			}
		}
		return result;
	}
