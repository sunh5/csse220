...
public class MonsterR extends Monster {
	private Color color;
	...
	public MonsterR(GameWorld world, Point2D drawPoint) {
...
//			this.bounce = -this.bounce;
			...
			double dx = this.getDrawPoint().getX() + this.bouncex*this.setMove();
			double dy = this.getDrawPoint().getY() + this.bounce*this.setMove();
			if(dx + 40 >= 1000) dx = 0;
			if(dx < 0) dx = 960;
			if(withinWall(dx,dy) == true) this.bouncex = -this.bouncex;
			point = new Point2D.Double(dx, dy);
