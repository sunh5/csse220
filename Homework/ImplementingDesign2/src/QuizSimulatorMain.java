...
public class QuizSimulatorMain {
	...
	private HashMap<Integer, Quiz> quiz = new HashMap<Integer, Quiz>();
	private HashMap<Integer, QuizTester> quizTesters = new HashMap<Integer, QuizTester>();
	...
	public void handleCreateQuestion(int id, boolean answer, String prompt) {
		...
		Question questions  = new Question (id, answer, prompt);
	...
	public String handleGetQuestionPrompt(int questionId) {
		...
		for (int i : this.questions.keySet()) {
			if (i == questionId) {
				return this.questions.get(i).getPrompt();
			}
		}
		return null;
	}
...
//		ArrayList<Question> allQuestion = new ArrayList<Question>();
		Quiz quizs = new Quiz (id);
		this.quiz.put(id, quizs);
	}
	/**
...
//		for (int i : this.questions.keySet()) {
//			if (i == questionId) {
//				allQuestions.add(this.questions.get(i));
//			}
//		}
//		Quiz quiz1 = new Quiz(quizId);
//		quiz1.addQuestion(questionId, allQuestions);
		this.quiz.get(quizId).addQuestion(questionId, this.questions.get(questionId));
	}
	/**
	...
	public void handleUpdateQuestion(int questionId, boolean answer, String prompt) {
		...
		for (int i : this.questions.keySet()) {
			if (i == questionId) {
				this.questions.get(i).updateQuestion(answer, prompt);
			}
		}
	}
	/**
	...
	public String handleGetQuizString(int quizId) {
		//TODO complete this method
		return this.quiz.get(quizId).getQuestions();
	...
	public void handleCreateQuizTester(int id, String searchString) {
		//TODO complete this method
		QuizTester testers = new QuizTester(id, searchString);
		this.quizTesters.put(id, testers);
	}
	/**
	...
	public double handleGetQuizTesterScoreOnQuiz( int quizTesterId, int quizId) {
		//TODO complete this method
		return this.quizTesters.get(quizTesterId).getScorePrecent(this.quiz.get(quizId));
	}
	/**
	...
	public double handleGetQuizTesterOverallScore(int quizTesterId) {
		//TODO complete this method
		double score = 0;
		for(int i:this.quiz.keySet()){
			score+=this.quizTesters.get(quizTesterId).getScorePrecent(this.quiz.get(i));
		}
		return score/this.quiz.size();
	}
