...
public class TeamGradebook {
	...
	private String handleAddStudent(String studentName) {
		...
		for (Student member : this.students) {
			if (member.getName().equals(studentName)) {
	...
	private String handleGetAbsences(String studentName) {
		for (Student stu : this.students) {
			if (stu.getName().equals(studentName)) {
				int i = stu.getAbsence();
				String numOfAbsence = Integer.toString(i);
				return numOfAbsence;
			}
		}
		return "error";
	...
	private String handleAddTeam(String teamName, ArrayList<String> memberNames) {
		...
		// TODO: Your code here
		Team newTeam = new Team(teamName, memberNames);
		for (Team tName : this.teams) {
			if (tName.getName().equals(teamName)) {
				return "Team Exist";

			}
		}
		this.teams.add(newTeam);
		newTeam.addMember(memberNames);
		for (String str : memberNames) {
			if (getStudentByName(str) == null) {
				handleAddStudent(str);
			}
		}
	...
	private String handleAddGrade(String teamName, double grade) {
		// TODO: Your code here
//		
		for (Team tea : this.teams) {
			if (tea.getName().equals(teamName)) {
				for (String name : tea.getMember()) {
					for (Student str : this.students) {
						if (str.getName().equals(name)) {
							str.addGrade(grade);
							tea.addGrade(grade);
							tea.addnumberOfGrades();
							return "ok";
						}
					}
				}
	...
	private String handleGetAverage(String studentName) {
		...
		for (int i = 0; i < this.teams.size(); i++) {
			if (this.teams.get(i).getMember().contains(studentName)) {
				for (int j = 0; j < this.teams.get(i).getGrade().size(); j++) {
					score += this.teams.get(i).getGrade().get(j);
					count += 1;
				}
				
			}
		}
		String average = Integer.toString((int) Math.round(score / count));
		return average;
	}
	...
	private String handleGetBestTeam() {
		double maxGrade = 0;
		String maxTeam = "error";
		double score = 0;
		double count = 0;
		
		for (int i = 0; i < this.teams.size(); i++) {
			Team tea = this.teams.get(i);
			if (tea.getAvegrade() > maxGrade) {
				maxGrade = tea.getAvegrade();
				maxTeam = tea.getName();
			}
