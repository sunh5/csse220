...
public class Recursion {
	...
	public static int findThirdCapital(String input) {
		int[] res = findThirdCapitalHelper(input,0);
		return res[3];
	}
	public static int[] findThirdCapitalHelper(String input, int currentLoc) {
		int[]res = new int[3];
		if (input.length() == 0) return res[-1,-1,-1];
		else {
			
		}
//		if (currentLoc == -1) return -1;
//		 else {
//			int nextLoc = findThirdCapitalHelper(input, currentLoc -1);
//			if (nextLoc >0) {
//				System.out.print(nextLoc);
//				return nextLoc;
//			} else if (Character.isUpperCase(input.charAt(currentLoc))) {
//				return currentLoc;
//			} else {
//				return -1;
//			}
//		}
	...
	public static void flipAfterNegative(int[] input) {
		flipAfterNegativeHelper(input, 0);
	}
	public static int[] flipAfterNegativeHelper(int[] input, int currentLoc) {
		if (input.length == 0) return input;
		if (currentLoc < input.length) {
			if (input[currentLoc] > 0) {
				flipAfterNegativeHelper(input, currentLoc++);
			}
			else if (input[currentLoc] < 0) {
				flipAfterNegativeHelper2(input,currentLoc+1);
...
//				return input;
	...
	public static void flipAfterNegativeHelper2(int[] input, int currentLoc) {
		while(currentLoc < input.length) {
			input[currentLoc] = -input[currentLoc];
			currentLoc++;
		}
	}
	/**
