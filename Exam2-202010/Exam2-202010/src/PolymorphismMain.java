...
//	ArrayList<LessTimidCapitializer> capitalizers;
	ArrayListArrayList<LessTimid> LessTimid;
	Random random;
	...
	public void setupAllModifiers() {
		LessTimid = new ArrayList<LessTimid>();
//		replacers = new ArrayList<LessTimidReplace>();
//		capitalizers = new ArrayList<LessTimidCapitializer>();
		
		LessTimid.add(new LessTimidAddCharacters("!!!!!"));
		LessTimid.add(new LessTimidAddCharacters("OR ELSE"));
		LessTimid.add(new LessTimidAddCharacters(", stinkybutt"));
		
		LessTimid.add(new LessTimidReplace(" ", "_"));
		LessTimid.add(new LessTimidReplace("?", "!!!"));
		LessTimid.add(new LessTimidReplace(":)", ":-P"));
	
		LessTimid.add(new LessTimidCapitializer());
	}
	
	public String applyRandomEnhancement(String input) {
		int numEnhancements = LessTimid.size();// + replacers.size() + capitalizers.size();
		int pick = random.nextInt(numEnhancements);
		if(pick < LessTimid.size()) {
			return LessTimid.get(pick).applyToString(input);
		}
//		pick -= adders.size();
...
//		
	...
	public void printDescriptions() {
		...
		int currentNum = 1;
		for(LessTimidAddCharacters adder : LessTimid) {
